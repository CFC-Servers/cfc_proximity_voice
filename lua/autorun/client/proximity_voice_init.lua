...
local canvarHearDead = CreateClientConVar( "proximity_voice_hear_dead", "0", true, true )
...
local canvar3D = CreateClientConVar( "proximity_voice_3D", "0", true, true )
...
local function populatePanel( form )
    ...
    hearDeadCheckbox:SetChecked( canvarHearDead:GetBool() )

    local checkbox3D = form:CheckBox( "3D Sound", "proximity_voice_3D" )
    checkbox3D:SetChecked( canvar3D:GetBool() )
...
hook.Add( "InitPostEntity", "CFC_ProximityVoice_SendConfigToServer", function()
    net.Start( "proximity_voice_settings_changed" )
        ...
        net.WriteBool( canvarHearDead:GetBool() )
        net.WriteBool( canvar3D:GetBool() )
